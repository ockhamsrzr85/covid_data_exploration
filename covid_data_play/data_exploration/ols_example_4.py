...
def run(min_date, max_date):
    ...
    X = aggr['max(norm_prob_of_dying_30_70_from_any_of_cardiovascular_cancer_diabetes_chronic_respiratory)']
    y = aggr['max(norm_pop_over_65_per_100_2019)']

    plt.scatter(X, y, c=normalized_colors, cmap='Reds', s=400)

    plt.title("Total deaths / Total Cases")
    plt.xlabel('norm_prob_of_dying_30_70_from_any_of_cardiovascular_cancer_diabetes_chronic_respiratory')
    plt.ylabel('norm_pop_over_65_per_100_2019')
